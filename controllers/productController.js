...
async function getAllProducts(res, limit) {
  ...
  let query;
  if (limit >= 1) {
    query = `SELECT product_id, name, FORMAT(price,2) as price, image FROM products LIMIT ${limit}`;
  } else {
    query =
      "SELECT product_id, name, FORMAT(price,2) as price, image FROM products";
  }
